...
import { 
  ...
  History, Plus, Edit, Trash2, Copy, Eye, X, 
  Loader2, Briefcase, Phone
} from 'lucide-react'
...
export default function ClientDetailPage() {
  ...
  async function fetchData() {
    // --- CORRECCIÓN FINAL: Validación Robustez ---
    if (!id) return; 
    
    // Intentamos convertir a número
    const clientIdNum = Number(id);

    // Si no es un número válido (ej: 'undefined', 'nuevo', 'NaN'), detenemos todo suavemente
    ...
    try {
        setLoading(true); // Aseguramos loading activo al iniciar petición
        ...
        setErrorMsg(null); // Limpiamos errores previos si hubo éxito
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6 bg-slate-50 min-h-screen font-sans text-slate-600 relative">
      
      {/* HEADER */}
      <div className="flex justify-between items-center bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
        <div>
          <h1 className="text-3xl font-black text-slate-800 uppercase tracking-tighter leading-none">{client?.empresa}</h1>
        ...
        <button onClick={() => router.push('/asesor')} className="flex items-center gap-2 bg-white border border-slate-200 px-4 py-2 rounded-xl font-bold text-xs hover:bg-slate-50 transition text-slate-600 shadow-sm"><ArrowLeft size={16} /> VOLVER</button>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    {contacts.map(c => (
                        <div key={c.id} className="p-3 bg-slate-50 rounded-lg border border-slate-100 flex justify-between items-center group hover:bg-blue-50/50 transition-colors">
                            <div className="flex-1">
                                <p className="text-[11px] font-black text-slate-800 uppercase leading-none mb-1">{c.nombre}</p>
                                <p className="text-[10px] text-slate-500">{c.correo}</p>
                                {c.telefono && (
                            ...
                            <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity ml-2">
                                <button onClick={() => handleEditContact(c)} className="p-1.5 hover:bg-white text-slate-300 hover:text-blue-600 rounded shadow-sm border border-transparent hover:border-slate-100"><Edit size={14}/></button>
                                <button onClick={() => handleDeleteContact(c.id)} className="p-1.5 hover:bg-white text-slate-300 hover:text-red-600 rounded shadow-sm border border-transparent hover:border-slate-100"><Trash2 size={14}/></button>
                            </div>
                        </div>
                    ))}
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          {/* ACORDEON COTIZACIONES */}
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            <div className="p-5 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
              <div className="flex items-center gap-3"><div className="bg-blue-100 p-2 rounded-lg text-blue-600"><FileText size={18} /></div><div><h2 className="text-sm font-black text-slate-700 uppercase tracking-tight">Cotizaciones</h2><p className="text-[10px] text-slate-400 font-bold uppercase">{quotes.length} Registros</p></div></div>
            ...
            <div className="grid grid-cols-12 gap-2 px-6 py-3 bg-slate-50 border-b border-slate-100 text-[9px] font-black uppercase text-slate-400 tracking-wider">
            ...
            <div className="divide-y divide-slate-100">
              {quotes.slice(0, visibleQuotes).map(q => (
                <details key={q.id} className="group">
                  <summary className="grid grid-cols-12 gap-2 items-center px-6 py-3 cursor-pointer hover:bg-blue-50/20 transition-colors list-none">
                  ...
                  <div className="px-14 pb-4 bg-blue-50/10">
                     <div className="text-[10px] p-3 rounded-lg border border-slate-200 bg-white">
