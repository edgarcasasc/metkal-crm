...
import { Save, Plus, Trash2, Loader2, FileText, Sun } from 'lucide-react'
...
interface OpticsParameter {
    id: string;
    nombre: string;
    puntos: OpticsPoint[];
}

export default function OpticsCalibrationForm({ itemId, orderId, magnitud }: { itemId: number, orderId?: string, magnitud: string }) {
    ...
    const [config, setConfig] = useState({
        ...
        intervalo_max: 4000,
        resolucion: 0.1,
        unidad: magnitud === 'Frecuencia' ? 'Hz' : 'Lux', // Dependerá si lo abrieron como Óptica o Frecuencia
        especificacion: '',
        distribucion: 'rectangular'
    })

    const createEmptyPoint = (): OpticsPoint => ({
        nominal: '', toma1: '', toma2: '', toma3: '', toma4: '', toma5: '',
        tol_patron: 0, cmc: 2,
        promedio: 0, error: 0, uA: 0, uB_patron: 0, uB_resol: 0, uC: 0, U_expandida: 0, error_abs: 0
    })

    const [parameters, setParameters] = useState<OpticsParameter[]>([
        {
            id: 'param-1',
            nombre: magnitud.toUpperCase(),
            puntos: [
                { ...createEmptyPoint(), nominal: '100.20', tol_patron: 2.35 },
                { ...createEmptyPoint(), nominal: '1014.00', tol_patron: 0.65 },
                { ...createEmptyPoint(), nominal: '2320.00', tol_patron: 0.95 },
                { ...createEmptyPoint(), nominal: '3527.00', tol_patron: 2.60 },
                { ...createEmptyPoint(), nominal: '4000.00', tol_patron: 2.60 }
            ]
        }
    ])
    ...
    const handlePointChange = (pIdx: number, ptIndex: number, field: keyof OpticsPoint, value: any) => {
        setParameters(prev => {
            const arr = [...prev]
    ...
    const recalculatePoint = (pt: OpticsPoint, cfg: typeof config): OpticsPoint => {
        ...
        if (tomas.filter(t => !isNaN(t)).length === 5) {
            ...
            // Fórmulas idénticas al Excel O-MK-0285
            const uA = stdDev * 1.4; 
            const uB_patron = pt.tol_patron / 2; // OJO: Usa la tol_patron ESPECÍFICA de esta fila
            const uB_resol = cfg.resolucion / Math.sqrt(12); // Resolucion / sqrt(12) -- matemáticamente igual a (Res/(2*sqrt(3)))
    ...
    const handleSave = async () => {
        ...
        try {
            const { error: resError } = await supabase.from('calibration_results').insert({
                ...
                observaciones: anexo,
                datos_calibracion: {
                    tipo: magnitud.toUpperCase(), // FRECUENCIA u OPTICA
            ...
            await supabase.from('service_order_items').update({ 
                ...
                magnitud: magnitud // Ej. 'Frecuencia' o 'Optica'
            }).eq('id', itemId)

            alert(`✅ Calibración de ${magnitud} guardada con éxito.`)
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden text-[10px]">
            ...
            <div className="bg-amber-50 p-4 border-b border-amber-100 flex justify-between items-center">
                <h3 className="font-bold text-amber-800 uppercase flex items-center gap-2 text-sm">
                    <Sun size={18} className="text-amber-600"/> HOJA DE CÁLCULO {magnitud.toUpperCase()}
                </h3>
            </div>
            ...
            <div className="p-4 space-y-8">
                        ...
                        <div key={param.id} className="border-2 border-black">
                            ...
                            <div className="bg-white text-black p-2 text-center border-b-2 border-black flex items-center">
                                ...
                                <input type="text" className="w-full text-center font-bold text-lg uppercase outline-none" 
                                    value={param.nombre} onChange={(e) => updateParameterField(pIdx, 'nombre', e.target.value)} placeholder="FRECUENCIA" />
                            </div>
                                    ...
                                    <table className="text-center text-[10px] whitespace-nowrap border-collapse">
                                        <thead className="font-bold bg-white">
                                            <tr>
                                                ...
                                                <th className="border-r border-black p-1 align-bottom bg-yellow-50 text-blue-800">cmc</th>
                                        ...
                                        <tbody className="border-t-2 border-black">
                                            {param.puntos.map((row, i) => (
                                                <tr key={i} className="bg-white border-b border-slate-300">
                                                    ...
                                                    {/* CMC DINÁMICA */}
                                                    <td className="border-r border-black p-0 bg-yellow-100">
                                                        <input type="number" className="w-12 text-center bg-transparent outline-none font-bold text-blue-700" value={row.cmc} onChange={e => handlePointChange(pIdx, i, 'cmc', parseFloat(e.target.value))} step="0.01"/>
                                                    </td>
                ...
                <div className="bg-slate-50 border border-slate-200 p-4 rounded-xl">
                    ...
                    <textarea 
                        ...
                        placeholder="Ej: Método de Comparación directa..."
                        value={anexo}
