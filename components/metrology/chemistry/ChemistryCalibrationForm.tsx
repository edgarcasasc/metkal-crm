...
import { Save, Plus, Trash2, Loader2, FileText, FlaskConical } from 'lucide-react'
...
interface ChemistryPoint {
    ...
    incertidumbre: number; // Editable por fila
    promedio: number; error: number;
    uA: number; uB_patron: number; uB_resol: number; uC: number;
    U_expandida: number; error_abs: number;
}

interface ChemistryParameter {
    id: string;
    nombre: string;
    puntos: ChemistryPoint[];
}

export default function ChemistryCalibrationForm({ itemId, orderId }: { itemId: number, orderId?: string }) {
    ...
    const [config, setConfig] = useState({
        ...
        intervalo_max: 1251,
        resolucion: 0.1,
        unidad: 'µS/cm', // Por defecto para conductividad, pero editable
        especificacion: '',
        distribucion: 'rectangular'
    })

    const createEmptyPoint = (): ChemistryPoint => ({
        nominal: '', toma1: '', toma2: '', toma3: '', toma4: '', toma5: '',
        incertidumbre: 0.0577,
        promedio: 0, error: 0, uA: 0, uB_patron: 0, uB_resol: 0, uC: 0, U_expandida: 0, error_abs: 0
    })

    const [parameters, setParameters] = useState<ChemistryParameter[]>([
        {
            id: 'param-1',
            nombre: 'QUÍMICA',
            puntos: [
                { ...createEmptyPoint(), nominal: '78.00', incertidumbre: 0.0577 },
                { ...createEmptyPoint(), nominal: '1251.00', incertidumbre: 0.0577 }
            ]
        }
    ])
    ...
    const handlePointChange = (pIdx: number, ptIndex: number, field: keyof ChemistryPoint, value: any) => {
        setParameters(prev => {
            const arr = [...prev]
    ...
    const recalculatePoint = (pt: ChemistryPoint, cfg: typeof config): ChemistryPoint => {
        ...
        if (tomas.filter(t => !isNaN(t)).length === 5) {
            ...
            // Fórmulas idénticas al Excel Q-MK-0157
            const uA = stdDev * 1.4; 
            const uB_patron = pt.incertidumbre / Math.sqrt(3); // En Química se divide por raíz de 3
            const uB_resol = cfg.resolucion / Math.sqrt(12); // Resolucion / sqrt(12)
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            alert("✅ Calibración de Química guardada con éxito.")
    ...
    return (
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden text-[10px]">
            ...
            <div className="bg-emerald-50 p-4 border-b border-emerald-100 flex justify-between items-center">
                <h3 className="font-bold text-emerald-800 uppercase flex items-center gap-2 text-sm">
                    <FlaskConical size={18} className="text-emerald-600"/> HOJA DE CÁLCULO QUÍMICA
                </h3>
            </div>
            ...
            <div className="p-4 space-y-8">
                        ...
                        <div key={param.id} className="border-2 border-black">
                            ...
                            <div className="bg-white text-black p-2 text-center border-b-2 border-black flex items-center">
                                ...
                                <input type="text" className="w-full text-center font-bold text-lg uppercase outline-none" 
                                    value={param.nombre} onChange={(e) => updateParameterField(pIdx, 'nombre', e.target.value)} placeholder="QUÍMICA" />
                            </div>
                                    ...
                                    <table className="text-center text-[10px] whitespace-nowrap border-collapse">
                                        <thead className="font-bold bg-white">
                                            <tr>
                                                ...
                                                {/* INCERTIDUMBRE AHORA ES COLUMNA (Antes Tol. de Patrón) */}
                                                <th className="border-r border-black p-1 align-bottom bg-yellow-50 text-blue-800">Incertidumbre<br/>{config.unidad}</th>
                                            ...
                                            {param.puntos.map((row, i) => (
                                                <tr key={i} className="bg-white border-b border-slate-300">
                                                    ...
                                                    <td className="border-r border-black p-0 bg-yellow-100">
                                                        <input type="number" className="w-16 text-center bg-transparent outline-none font-bold text-blue-700" value={row.incertidumbre} onChange={e => handlePointChange(pIdx, i, 'incertidumbre', parseFloat(e.target.value))} step="0.0001"/>
                                                    </td>
                ...
                <div className="bg-slate-50 border border-slate-200 p-4 rounded-xl">
                    ...
                    <textarea 
                        ...
                        placeholder="Ej: Método de Comparación..."
                        value={anexo}
